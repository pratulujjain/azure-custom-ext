...
resource "azurerm_storage_account" "vmstore4577689" {
  name                     = "vmstore4577689"
...
resource "azurerm_storage_container" "data" {
  name                  = "data"
  storage_account_name  = "vmstore4577689"
  container_access_type = "blob"
  depends_on=[
    azurerm_storage_account.vmstore4577689
    ]
}
...
resource "azurerm_virtual_machine_extension" "vmextension" {
  ...
  settings = <<SETTINGS
    {
        "fileUris": ["https://${azurerm_storage_account.vmstore4577689.name}.blob.core.windows.net/data/IIS_Config.ps1"],
